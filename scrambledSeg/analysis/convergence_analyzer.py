...
class MultiMetricConvergenceAnalyzer:
    ...
    def should_stop_training(
        ...
        if self.global_convergence_state.get("plateau_ratio", 0) >= plateau_threshold:
            logger.info(
                f"Training plateau detected: {self.global_convergence_state['plateau_ratio']:.2%} of metrics plateaued"
            )
            return True
...
class ConvergenceCallback(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        ...
        should_stop = self.analyzer.should_stop_training(
            min_epochs=self.min_epochs, convergence_threshold=0.8, plateau_threshold=0.6
        )
