...
class MetricsAnalyzer:
    ...
    def set_class_names(self, class_names: List[str]) -> None:
        ...
        self._class_column_suffix = {name: self._to_column_suffix(name) for name in class_names}
    ...
    def _estimate_convergence_epoch(
        self, series: pd.Series, metric_name: str, window_size: int = 10
    ) -> Optional[int]:
    ...
    def analyze_generalization_gap(self) -> dict[str, float]:
        ...
        for metric in base_metrics:
            ...
            if train_col in self.df.columns and val_col in self.df.columns:
                ...
                train_final = (
                    self.df[train_col].dropna().iloc[-1]
                    if not self.df[train_col].dropna().empty
                    else np.nan
                )
                val_final = (
                    self.df[val_col].dropna().iloc[-1]
                    if not self.df[val_col].dropna().empty
                    else np.nan
                )
                ...
                if len(train_data) > 0 and len(val_data) > 0:
                    ...
                    if len(common_indices) > 1:
                        ...
                        tests[f"{metric}_ttest"] = {
                            ...
                            "interpretation": "Significant difference between train and val"
                            if p_value < 0.05
                ...
                for split in ["train", "val"]:
                    ...
                    if col_name in self.df.columns:
                        summary = self._calculate_metric_summary(
                            self.df[col_name], f"{class_name}_{split}_{metric_type}"
                        )
    ...
    def _generate_markdown_report(self, analysis: TrainingAnalysis, filepath: Path):
        ...
        with open(filepath, "w") as f:
            ...
            if analysis.training_time_minutes:
                f.write(
                    f"- **Training Time**: {analysis.training_time_minutes:.1f} minutes ({analysis.training_time_minutes / 60:.1f} hours)\\n"
                )
            f.write("\\n")
            ...
            f.write(
                "| Metric | Final Value | Best Value | Best Epoch | Improvement | Convergence Epoch |\\n"
            )
            f.write(
                "|--------|-------------|------------|------------|-------------|-------------------|\\n"
            )
            ...
            for metric_name, summary in analysis.metric_summaries.items():
                if not np.isnan(summary.final_value):
                    f.write(
                        f"| {metric_name} | {summary.final_value:.4f} | {summary.best_value:.4f} | {summary.best_epoch} | {summary.improvement:+.4f} | {summary.convergence_epoch or 'N/A'} |\\n"
                    )
            f.write("\\n")
            ...
            if analysis.statistical_tests:
                ...
                for test_name, test_result in analysis.statistical_tests.items():
                    ...
                    f.write(
                        f"- **Interpretation**: {test_result.get('interpretation', 'N/A')}\\n\\n"
                    )
            ...
            for class_name, class_metrics in analysis.per_class_analysis.items():
                ...
                for metric_name, summary in class_metrics.items():
                    if not np.isnan(summary.final_value):
                        f.write(
                            f"| {metric_name} | {summary.final_value:.4f} | {summary.best_value:.4f} | {summary.improvement:+.4f} |\\n"
                        )
                f.write("\\n")
    ...
    def export_for_paper(self, output_dir: str, format_for_latex: bool = True) -> dict[str, Path]:
        ...
        table_file = output_dir / (
            "paper_metrics_table.tex" if format_for_latex else "paper_metrics_table.csv"
        )
    ...
    def _create_paper_table(
        self, analysis: TrainingAnalysis, latex_format: bool = True
    ) -> PaperTable:
    ...
    def _extract_key_statistics(self, analysis: TrainingAnalysis) -> KeyStatistics:
        ...
        stats: KeyStatistics = {
            ...
            "training_time_hours": analysis.training_time_minutes / 60
            if analysis.training_time_minutes
            else None,
            "final_val_iou": None,
