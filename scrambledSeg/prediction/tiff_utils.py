...
class TiffHandler:
    ...
    def save_tiff(self, data: np.ndarray, path: PathLike, threshold: float = 0.5) -> None:
        ...
        logger.info(
            f"Saved image to {path} with shape {data.shape} and values in range [{np.min(data)}-{np.max(data)}]"
        )
    ...
    def merge_tiles(
        ...
        if is_multiclass:
            logger.info(
                "Detected multi-class segmentation, using special merging for class indices"
            )
    ...
    def iter_tiles(self, image: np.ndarray) -> Iterator[TiledPrediction]:
        ...
        if len(image.shape) == 3 and image.shape[2] > 10:  # Likely a 3D volume incorrectly passed
            raise ValueError(
                f"Received 3D volume with shape {image.shape}. Use predict_image with 3D handling instead."
            )
