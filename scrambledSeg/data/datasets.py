...
class SynchrotronDataset(Dataset):
    ...
    def _initialize_dataset_metadata(self):
        ...
        if self._has_multiclass_labels(all_unique_values):
            logger.info(
                f"Detected multi-class segmentation with {len(all_unique_values)} unique values across dataset: {all_unique_values}"
            )
    ...
    def _load_from_file(self, idx: int) -> ArraySample:
        ...
        if self._has_multiclass_labels(unique_values):
            ...
            if self.multi_class is None:
                self.multi_class = True
                logger.info(
                    f"Detected multi-class segmentation with {len(unique_values)} unique values: {unique_values}"
                )
        else:
        ...
        if self.data_shape is None:
            ...
            raise ValueError(
                f"Data shape mismatch at index {idx}. Expected {self.data_shape}, got {data.shape}"
            )
        elif label.shape != self.label_shape:
            raise ValueError(
                f"Label shape mismatch at index {idx}. Expected {self.label_shape}, got {label.shape}"
            )
    ...
    def __getitem__(self, idx: int) -> TensorSample:
        ...
        try:
            ...
            if sample is None:
                ...
                sample = {"image": sample["image"].copy(), "mask": sample["mask"].copy()}
