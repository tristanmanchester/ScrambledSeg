        ...
        if pred.dim() == 4:
            ...
            if self.per_image:
                ...
                for i in range(B):
                    ...
                    losses.append(
                        lovasz_softmax_flat(img_pred, img_target, self.classes, self.ignore)
                    )
...
class BCEDiceLoss(nn.Module):
    def __init__(self, bce_weight: float = 0.5, dice_weight: float = 0.5, smooth: float = 1.0):
        """Combine BCE-with-logits and Dice for binary segmentation."""
        super().__init__()
    ...
    def dice_loss(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """Compute the Dice component from raw logits."""
        pred = torch.sigmoid(pred)
        dims = (2, 3) if pred.dim() == 4 else (1, 2)
        intersection = (pred * target).sum(dims)
        cardinality = (pred + target).sum(dims)

        dice = 1 - ((2.0 * intersection + self.smooth) / (cardinality + self.smooth))
        return dice.mean()

    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """Return the weighted BCE-plus-Dice loss."""
...
class TverskyLoss(nn.Module):
    ...
    def __init__(
        self, alpha: float = 0.5, beta: float = 0.5, smooth: float = 1.0, ignore_index: int = -100
    ):
        """
        Args:
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """Compute Tversky loss from logits and class-index targets."""
        if targets.dim() == 4 and targets.size(1) == 1:
            targets = targets.squeeze(1)
...
class CompoundLoss(nn.Module):
    """Combine Lovasz, focal, and Tversky losses for multi-class segmentation."""
    ...
    def __init__(
        ...
        """Configure the compound loss weights and component settings."""
        super().__init__()
        ...
        self.tversky_loss = TverskyLoss(
            alpha=tversky_alpha, beta=tversky_beta, smooth=smooth, ignore_index=ignore_index
        )

        weights_sum = lovasz_weight + focal_weight + tversky_weight
        if abs(weights_sum - 1.0) > 1e-6:
            logger.warning(
                f"Loss weights sum to {weights_sum} instead of 1.0. Normalizing weights."
            )
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """Return the weighted sum of all component losses."""
        ...
        if torch.isnan(lovasz) or torch.isnan(focal) or torch.isnan(tversky):
            logger.error(
                f"NaN in losses: Lovász={lovasz.item()}, "
                f"Focal={focal.item()}, Tversky={tversky.item()}"
            )

        return (
            self.lovasz_weight * lovasz + self.focal_weight * focal + self.tversky_weight * tversky
        )
...
class CrossEntropyDiceLoss(nn.Module):
    ...
    def multi_class_dice_loss(
        ...
        dice_per_class = 1 - ((2.0 * intersection + self.smooth) / (cardinality + self.smooth))
...
def _normalize_loss_type(loss_type: str) -> str:
    """Normalize hyphenated and legacy loss aliases into one loss key."""

    normalized = loss_type.lower().replace("-", "_")
    alias_map = {
        "bce_dice": "bcedice",
        "cross_entropy": "crossentropy",
        "cross_entropy_dice": "crossentropy_dice",
    }
    return alias_map.get(normalized, normalized)


def _resolve_cross_entropy_dice_weights(
    *,
    ce_weight: float = 0.5,
    bce_weight: Optional[float] = None,
) -> float:
    """Resolve the legacy ``bce_weight`` alias for ``CrossEntropyDiceLoss``."""

    if bce_weight is not None:
        if ce_weight != 0.5 and ce_weight != bce_weight:
            raise ValueError(
                "Received both ce_weight and the legacy bce_weight alias with "
                "different values. Use only ce_weight for CrossEntropyDiceLoss."
            )
        ce_weight = bce_weight
    return ce_weight


def create_loss(loss_type: str = "crossentropy_dice", **kwargs) -> nn.Module:
    """Instantiate a configured loss from its normalized name."""

    normalized = _normalize_loss_type(loss_type)

    if normalized == "lovasz":
        return LovaszSoftmaxLoss(**kwargs)
    if normalized == "bcedice":
    ...
    if normalized == "crossentropy_dice":
        ce_weight = _resolve_cross_entropy_dice_weights(
            ce_weight=kwargs.pop("ce_weight", 0.5),
            bce_weight=kwargs.pop("bce_weight", None),
        )
        return CrossEntropyDiceLoss(ce_weight=ce_weight, **kwargs)
    if normalized == "crossentropy":
...
def list_available_losses() -> List[str]:
    ...
    return sorted(AVAILABLE_LOSS_TYPES)
