...
class SegmentationVisualizer:
    def __init__(
        ...
        if self.base_dir:
            ...
            self.metrics_file = (
                Path(metrics_file) if metrics_file else (self.metrics_dir / "metrics.csv")
            )
    ...
    def _normalize_tensor(
        self, tensor: Union[torch.Tensor, np.ndarray]
    ...
    def find_interesting_slices(self, masks, k: int = 4) -> List[int]:
        ...
        coverages = [
            (i, coverage) for i, coverage in all_coverages if coverage >= self.min_coverage
        ]
    ...
    def visualize_prediction(
        ...
        try:
            ...
            n_output_classes = (
                prediction.shape[1] if torch.is_tensor(prediction) else prediction.shape[1]
            )

            # Select which class probabilities to show (first two non-background classes if possible)
            prob_classes = []
            for c in range(
                1, min(n_output_classes, 3)
            ):  # Skip background (0), show up to 2 classes
            ...
            plot_configs = [
                ...
                (
                    prediction_softmax[0, prob_classes[0]],
                    f"Class {prob_classes[0]} Probability",
                    "viridis",
                    (0, 1),
                ),
                (
                    prediction_softmax[0, prob_classes[-1]],
                    f"Class {prob_classes[-1]} Probability",
                    "viridis",
                    (0, 1),
                ),
                (
    ...
    def plot_comprehensive_metrics(
        self, save_path: Optional[str] = None, window_size: int = 200, dpi: Optional[int] = None
    ) -> Optional[plt.Figure]:
        ...
        for metric, label in metrics_to_plot:
            ...
            if train_col in df.columns and not df[train_col].isna().all():
                smoothed_train = (
                    df[train_col].rolling(window=min(window_size, len(df)), min_periods=1).mean()
                )
                ax2.plot(
                    df["step"],
                    smoothed_train,
    ...
    def plot_per_class_metrics(
        ...
        per_class_columns = [
            col for col in df.columns if any(f"_{cls.lower()}" in col for cls in class_names)
        ]
        ...
        for idx, metric_type in enumerate(metric_types):
            ...
            for class_idx, (class_name, color) in enumerate(zip(class_names, colors)):
                ...
                if train_col in df.columns and not df[train_col].isna().all():
                    smoothed_train = (
                        df[train_col]
                        .rolling(window=min(window_size, len(df)), min_periods=1)
                        .mean()
                    )
                    ax.plot(
                        df["step"],
